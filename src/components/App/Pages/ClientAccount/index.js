...
class ClientAccountComponent extends Component {
  ...
  componentDidMount() {
    this.props.getPageClientOverview();
  }
  ...
  handleSearchSubmit = (event) => {
    ...
    this.props.getPageClient({ page: 1, s: search.text, from: search.from, to: search.to });
  }
    ...
    return (
      <Grid fluid className="contacts-page">
        ...
        {clientAccounts.total ? (
          <Pagination pagination={clientAccounts} route={juvo.Pages.client.pageLink} />
        ) : null}
      </Grid >
    );
  }
