...
// import Landlords from 'containers/App/Pages/Landlords';
import LandlordStatements from 'containers/App/Pages/LandlordStatements';
...
import Page from 'containers/App/Pages';
import PagesCreate from 'containers/App/Pages/PageCreate';
import PagesInfo from 'containers/App/Pages/PageInfo';
...
import Rents from 'containers/App/Pages/Rents';
import ClientAccount from 'containers/App/Pages/ClientAccount';
...
export default (store) => {
  ...
  const clientsOnLoad = (nextState, replace, callback) => {
    ...
    const page = (nextState.params && nextState.params.id) || 1;
    store.dispatch(getPagesClient({ page }));
  ...
  const rentsOnLoad = (nextState, replace, callback) => {
    store.dispatch(handleModalClose());
    const page = (nextState.params && nextState.params.id) || 1;
    store.dispatch(getPagesRents({ page, overdue: 1 }));
    const state = store.getState().common;
    if (!state.users || !state.users.length) {
      store.dispatch(getUsers());
    }
  ...
  //   callback();
  // };
  const loadPagesInfo = (nextState, replace, callback) => {
    store.dispatch(getPagesInfo(nextState.params && nextState.params.id));
    callback();
  };
  const clearPages = (nextState, replace, callback) => {
    store.dispatch(clearPagesInfo());
    callback();
  };
  const landlordStatementsOnLoad = (nextState, replace, callback) => {
    const data = {
      Pages_id: nextState.params.id,
      Pages_rent_due_id: nextState.params.rentId,
    };
    store.dispatch(getPagesInfo(nextState.params.id));
    store.dispatch(getPagesStatements(data));
    store.dispatch(getPagesStatementTemplate());
    callback();
  };
  ...
  return (
    <Route component={JuvoApp}>
      <Route path={juvo.index} component={App} onEnter={requireAuth}>
        ...
        <Route path={juvo.Page.index} component={Page} />
        <Route path={juvo.Page.create} component={PagesCreate} onEnter={clearPages} />
        <Route path={juvo.Page.client.index} component={ClientAccount} onEnter={clientsOnLoad} />
        <Route path={juvo.Page.client.pageRoute} component={ClientAccount} onEnter={clientsOnLoad} />
        <Route path={juvo.Page.rents.index} component={Rents} onEnter={rentsOnLoad} />
        <Route path={juvo.Page.rents.pageRoute} component={Rents} onEnter={rentsOnLoad} />
        <Route path={juvo.Page.rents.index} component={Rents} onEnter={rentsOnLoad} />
        <Route path={juvo.Page.rents.pageRoute} component={Rents} onEnter={rentsOnLoad} />
        <Route path={juvo.Page.landlords.index} component={Landlords} onEnter={landlordsOnLoad} />
        <Route path={juvo.Page.landlords.pageRoute} component={Landlords} onEnter={landlordsOnLoad} />
        <Route path={juvo.Page.landlords.statementsRoute} component={LandlordStatements} onEnter={landlordStatementsOnLoad} />
        <Route path={juvo.Page.infoRoute} component={PagesInfo} onEnter={loadPagesInfo} />
